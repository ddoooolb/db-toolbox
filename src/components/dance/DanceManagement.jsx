...
function DanceManagement() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'groups'), snapshot => {
      ...
      const groupsData = Object.keys(firestoreGroups).length > 0 ? firestoreGroups : initialGroupsData
  ...
  useEffect(() => {
    ...
    // Firestore에서 평가자 결과 읽기
    const unsubTeacherResults = onSnapshot(
      query(collection(db, 'dance-teacher-results'), where('classId', '==', selectedClass)),
      snapshot => {
        const results = {}
        snapshot.forEach(doc => {
    ...
    // Firestore에서 오버라이드 읽기
    const unsubOverrides = onSnapshot(
      query(collection(db, 'dance-overrides'), where('classId', '==', selectedClass)),
      snapshot => {
        const overrides = {}
        snapshot.forEach(doc => {
    ...
    // Firestore에서 결과 평가 오버라이드 읽기
    const unsubResultOverrides = onSnapshot(
      query(collection(db, 'dance-result-overrides'), where('classId', '==', selectedClass)),
      snapshot => {
